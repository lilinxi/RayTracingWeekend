...
Hitable *randomScene() {
    vector<Hitable *> list;
    list.push_back(new Sphere({0, -1000, 0}, 1000, new Lambertian(new ConstantTexture({0.5, 0.5, 0.5}))));
    for (int a = -11; a < 11; a++) {
        for (int b = -11; b < 11; b++) {
            ...
            if ((center - Vector3d{4, 0.2, 0}).norm() > 0.9) {
                if (chooseMat < 0.8) { // diffuse
                    list.push_back(
                            new MovingSphere(center, center + Vector3d{0, 0.5 * drand48(), 0},0.0,1.0, 0.2, new Lambertian(
                                    new ConstantTexture({drand48() * drand48(), drand48() * drand48(), drand48() * drand48()}))));
    ...
    list.push_back(new Sphere(Vector3d{4, 1, 0}, 1.0, new Metal(Vector3d{0.7, 0.6, 0.5},0.0)));
...
int main() {
    ...
    int ny = 720;
    int ns = 50;
    ofstream fout("test_motion_blur.ppm");
